...
if [ -f ".venv/bin/python" ]; then
    ...
    if [ -f "/Users/DevilDI/.local/bin/uv" ]; then
        PIP_CMD="/Users/DevilDI/.local/bin/uv pip"
    else
...
for mirror in "${MIRRORS[@]}"; do
    echo "尝试镜像: $mirror"
    if $PIP_CMD install pdfplumber -i "$mirror" --quiet; then
...
if [ "$INSTALLED" = false ]; then
    ...
    if $PIP_CMD install pdfplumber; then
        echo -e "${GREEN}✓ pdfplumber 安装成功！${NC}"
        INSTALLED=true
    else
        echo -e "${RED}✗ pdfplumber 安装失败${NC}"
        echo "请手动安装: $PIP_CMD install pdfplumber"
        exit 1
    fi
fi
...
echo "----------------------------------------"
$PYTHON_CMD -c "from backend.database.pdf_schema import init_pdf_tables; init_pdf_tables()" 2>/dev/null
if [ $? -eq 0 ]; then
...
echo "----------------------------------------"
$PYTHON_CMD -c "import pdfplumber; print(f'✓ pdfplumber 版本: {pdfplumber.__version__}')" 2>/dev/null
if [ $? -eq 0 ]; then
...
echo ""
echo "下一步操作："
echo "1. 启动后端: cd backend/api && ../../.venv/bin/python main.py"
